...
public class DepartmentExceptionHandler {
	...
	public ResponseEntity<?> noDepartmentFoundException(NoDepartmentFoundException e){
		return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ResponseStructure.builder().httpStatus(HttpStatus.NOT_FOUND.value()).message("No Department Found in database").body("Departments not present in database").build());
	}
